        ...
        // The side scrolling game!
            MyGame1 = ig.Game.extend({
                ...
                draw: function() {
                    ...
                    this.parent();
                    // gem
                    this.font.draw( 'Gems: ' + this.score_points, 6, 25,);
            
                    // this.statText.draw("Lives", 5,5); <-- this adds the "lives" text
                    for(var i=0; i < this.lives; i++)
                    this.lifeSprite.draw(((this.lifeSprite.width +2) * i)+5, 12);
                    // 'this.player' is set by the player's init method
                    // player exit
                    if(this.showStats){
                        this.statMatte.draw(0,0);
...
if( ig.ua.mobile ) {
	...
	myTouchButtons = new ig.TouchButtonCollection([
        ...
        new ig.TouchButton( 'switchup', {left: 0, top: 0}, 64, 64, buttonImage, 5 ),
		new ig.TouchButton( 'switchdown', {left: 0, top: 64}, 64, 64, buttonImage, 6 ),
        // A button

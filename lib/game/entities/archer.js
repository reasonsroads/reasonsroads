    ...
    speak: function() {
        if (Math.random() < .005) {
          ...
          if(!ig.game.getEntityByName(bubbleName)) {
            var parameters = {text: 'Iyam not as fast as you, but a straight shooter for sure!!', tracks: this.name, margin: 0, lifeSpan: 4, shape: 'square', name: bubbleName, color:[255,255,255]};
